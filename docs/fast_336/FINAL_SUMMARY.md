...
2026-08-18 的正式检测口径为 S600 `/dev/video0`、1280×720、30 FPS、336×336 `stretch`、`/detect box`、无 WebSocket。stable_672、Batch 1 和 Batch 2 各采集 120 秒稳态窗口；DDR 使用 `hrut_ddr -t rr_all -p 20000` 直接采集。完整阶段、资源时序和对比结果见：

- [Batch 1 推理优化](08_batch1_inference_optimization.md)
- [Batch 2 推理优化](09_batch2_inference_optimization.md)

严格时间窗输出为 stable_672 `1.717 FPS`（206 结果）、Batch 1 `7.991 FPS`（959 结果）和 Batch 2 `11.583 FPS`（1390 结果）。三轮均为 1 box、10 Token、3 次 PBD、`im_end`，推理错误为 0。该结果替代本页后续历史回灌数字作为当前实时吞吐口径；历史数据仅用于过程追溯。
